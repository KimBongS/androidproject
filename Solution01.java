...
public class Solution01 {
	...
	public static void main(String[] args) throws Exception {
		 ...
		 */
		System.setIn(new FileInputStream("sample_input01 - 복사본.txt"));
		...
		for(int test_case = 1; test_case <= T; test_case++) {
			...
			*/
			...
			for(int i =0;i<N;i++){
				//i번째 사람이 풀수있는 문제
				for(int j=0;j<P;j++){
					if(prob[j]!=true){//풀린 문제가 아니고
						if(N_arr[i][0]>=P_arr[j][0] && N_arr[i][1]>=P_arr[j][1] && N_arr[i][2]>=P_arr[j][2]){
							//점수 이상되는가
							score+=P_arr[j][3];
							prob[j] = true;
						}
